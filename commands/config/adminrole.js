...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('config-add-admin-role')
    .setDescription('Adds a role ID to adminRoles')
  ...
  async execute(interaction) {
    ...
    try {
      ...
      await interaction.reply({ embeds: [successEmbed('Config Updated', `Added <@&${role.id}> to adminRoles.`)], ephemeral: true });
    } catch (error) {
      console.error(error);
