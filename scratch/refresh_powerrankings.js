...
client.once('ready', async () => {
  console.log(`[POWER RANKINGS REFRESH] Logged in as ${client.user.tag}`);
  const guild = client.guilds.cache.get(config.guildId) || client.guilds.cache.first();
  if (guild) {
    await updatePowerRankingsMessage(guild);
    console.log('[POWER RANKINGS REFRESH] Updated power rankings message in server!');
  } else {
    console.error('[POWER RANKINGS REFRESH] Guild not found');
  }
  client.destroy();
  process.exit(0);
});
